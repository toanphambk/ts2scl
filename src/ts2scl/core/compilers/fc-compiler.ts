...
export class FunctionCompiler extends BaseCompiler {
    ...
    protected async getFCTypes(inputTypesDir: string): Promise<Function[]> {
        try {
            ...
            // Import the file with explicit handling for reflect-metadata
            ...
            // Filter for FC types
            const fcTypes = Object.values(inputTypes).filter(this.isFCType);
            this.logger.debug(`Found ${fcTypes.length} FC types`);
